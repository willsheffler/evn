...
def test_cherry_pick_import_from_subpackage(
        tmp_package: Tuple[str, str]) -> None:
    """Test cherry_pick_import from a subpackage."""
    path, pkg_name = tmp_package
    var2 = cherry_pick_import(f'{pkg_name}.subpkg.module2',
                              'variable2',
                              path=path)
    assert var2 == 42
...
def test_cherry_pick_import_class(tmp_package: Tuple[str, str]) -> None:
    ...
    path, pkg_name = tmp_package
    TestClass = cherry_pick_import(f'{pkg_name}.subpkg.module2',
                                   'TestClass',
                                   path=path)
...
def test_cherry_pick_import_nonexistent_module() -> None:
    ...
    with pytest.raises(ImportError):
        cherry_pick_import('nonexistent_package.nonexistent_module',
                           'some_attr')


def test_cherry_pick_import_nonexistent_attribute(
        tmp_package: Tuple[str, str]) -> None:
    """Test cherry_pick_import with a nonexistent attribute."""
    path, pkg_name = tmp_package

    with pytest.raises(AttributeError):
        cherry_pick_import(f'{pkg_name}.module1',
                           'nonexistent_attr',
                           path=path)


def test_cherry_pick_imports_multiple_attributes(
        tmp_package: Tuple[str, str]) -> None:
...
def test_cherry_pick_imports_empty_attributes(
        tmp_package: Tuple[str, str]) -> None:
...
def test_cherry_pick_imports_merged_attribute(
        tmp_package: Tuple[str, str]) -> None:
    path, pkg_name = tmp_package
...
def test_cherry_pick_imports_one_invalid_attribute(
        tmp_package: Tuple[str, str]) -> None:
    ...
    with pytest.raises((AttributeError, ImportError)):
        cherry_pick_imports(f'{pkg_name}.module1',
                            'variable1 nonexistent_attr')
...
def test_caching_behavior(tmp_package: Tuple[str, str]) -> None:
    ...
    var1_again = cherry_pick_import(f'{pkg_name}.module1',
                                    'variable1',
                                    path=path)
...
def test_circular_imports_simulation(tmp_package: Tuple[str, str]) -> None:
    ...
    with open(os.path.join(path, pkg_name, 'circular2.py'), 'w') as f:
        f.write(
            f"from {pkg_name}.circular1 import variable1\nvariable2 = f'circular2_var uses {{variable1}}'"
        )
...
def make_temp_package_structure() -> Generator[Tuple[str, str], None, None]:
    ...
    try:
        ...
        with open(os.path.join(temp_dir, pkg_name, 'module1.py'), 'w') as f:
            f.write(
                "variable1 = 'module1_var'\ndef func1(): return 'func1_result'"
            )

        with open(os.path.join(temp_dir, pkg_name, 'subpkg', 'module2.py'),
                  'w') as f:
            f.write(
