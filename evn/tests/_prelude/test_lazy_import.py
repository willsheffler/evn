...
def test_maybeimport():
    ...
    missing = evn.maybeimport(
        'noufuomemioixecmeiorutnaufoinairesvoraisevmraoui')
    assert not missing
    missing = evn.maybeimports(
        'noufuomem ioixecmeiorutnaufoina iresvoraisevmraoui')
...
def test_lazyimport_this():
    ...
    with evn.capture_stdio() as poem:
        assert this.c == 97
    assert 'The Zen of Python, by Tim Peters' == evn.first(
        poem.readlines()).strip()
