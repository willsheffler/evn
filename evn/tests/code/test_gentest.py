...
def test_qualname_of_file():
    ...
    for fname, expected in [
        ...
        at(evn.code.qualname_of_file(fname)).is_equal_to(expected)
...
def test_make_testfile(tmpdir):
    ...
    os.system(f'cp {evn.projconf} {tmpdir}')
    evn.code.make_testfile(sourcefile, testfile)
    code = pathlib.Path(testfile).read_text()
    code = code.replace('evn.code.gentest','sourcefile')
    ...
    diff = evn.console.run(f'diff -wB {tmpdir}/expected.py {tmpdir}/testfile.py', errok=True)
    if diff:
        print('-' * 80)
        print(code)
        print('-' * 80)
