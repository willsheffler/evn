...
def _test_func_ok(name, obj):
    return name.startswith('test_') and callable(
        obj) and evn.testing.no_pytest_skip(obj)


def _test_class_ok(name, obj):
    return name.startswith('Test') and isinstance(
        obj, type) and not hasattr(obj, '__unittest_skip__')
...
def maintest(namespace, config=evn.Bunch(), **kw):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for clsname, Suite in test_suites:
            ...
            test_methods = evn.meta.filter_namespace_funcs(
                vars(namespace[clsname]))
            test_methods = {
                k: v
                for k, v in test_methods.items() if _test_func_ok(k, v)
            }
            getattr(suite, 'setUp', lambda: None)()
            for name in test_methods:
                _maintest_run_maybe_parametrized_func(f'{clsname}.{name}',
                                                      getattr(suite, name),
                                                      result, config, kw)
            getattr(suite, 'tearDown', lambda: None)

        for name, func in test_funcs:
            _maintest_run_maybe_parametrized_func(name, func, result, config,
                                                  kw)

    if result.passed:
        print('PASSED   ', len(result.passed), 'tests')
    for label, tests in result.items():
        if label == 'passed' and not config.verbose and len(result.passed) > 7:
            continue
        for test in tests:
...
def _maintest_run_test_function(name,
                                func,
                                result,
                                config,
                                kw,
    ...
    if any([
        ...
        if testout:
            print(testout.read(), flush=True, end='')
        if config.nofail and error:
            print(error)
        elif error:
            raise error
